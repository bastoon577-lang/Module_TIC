...
/**
 * \fn void reboot_init(uint16_t in, unsigned long ms)
 */
void reboot_init(uint16_t in, unsigned long ms) {
  ...
  pinMode(reboot.pin,INPUT_PULLUP);
}
...
bool reboot_get_button_state(void) {
  bool state = digitalRead(reboot.pin);                   // Lecture de l'état de la broche
