 ...
 * \brief Fonction permettant la supression des données en mémoire EEPROM
 ...
 */
void html_generate_exploitation_page(void) {
  String str_to_write = "";
  ...
  web_server.sendContent(F("  }\n"));
  web_server.sendContent("  const portWS = "+String(static_conf.portWs)+";");
 ...
 */
void html_generate_configuration_page(void) {
  web_server.setContentLength(CONTENT_LENGTH_UNKNOWN);
  web_server.send(200,F("text/html"),"");
 ...
 */
void handle_action_exploitation_button(void) {
 ...
 */
void handle_action_configuration_button(void) {
  String post_data = web_server.arg("data");
 ...
 */
void handle_action_configuration_input(void) {
...
void setup() {
  ...
  reboot_init(GPIO_DMD_RESET,TIMEOUT_REBOOT);                           // Initialisation du service REBOOT
  ...
  if(static_conf.is_configured)                                         // L'equipement est-il configuré ?
  {
    ...
    led_init(GPIO_LED_IHM,TIMEOUT_LED_LENT);                            // Clignotement lent de la LED
  ...
  else                                                                  // L'equipement est vierge
  {
    ...
    led_init(GPIO_LED_IHM,TIMEOUT_LED_RAPI);                            // Clignotement rapide de la LED
  }
